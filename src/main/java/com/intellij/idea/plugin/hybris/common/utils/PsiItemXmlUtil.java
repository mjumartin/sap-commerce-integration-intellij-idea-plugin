...
public final class PsiItemXmlUtil {
    ...
    private static final Logger logger = Logger.getInstance(PsiItemXmlUtil.class);
    ...
    public static List<XmlElement> findTags(final PsiClass psiClass, final String tagName) {
        ...
        for (VirtualFile file : files) {
            final XmlFile xmlFile = (XmlFile) instance.findFile(file);
            if (xmlFile != null) {
                final DomFileElement<Items> domFile = manager.getFileElement(xmlFile, Items.class);

                if (domFile == null) {
                    logger.info("Skipping null domFile");
                    continue;
                }

                logger.info("DOM load for file " + xmlFile.getName() + " in " + xmlFile.getContainingDirectory());
                logger.info("DOM [" + domFile + "] for file " + file.getName() + "(" + file.getCanonicalPath() + ")");


//                assert domFile != null;
                final Items root = domFile.getRootElement();
